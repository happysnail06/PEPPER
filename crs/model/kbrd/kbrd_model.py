...
from crs.model.utils import SelfAttention, shift_tokens_right
...
class KBRDforConv(BartPretrainedModel, GenerationMixin):
    ...
    def generate(self, *args, **kwargs):
        """Delegate to GenerationMixin.generate now that the mix-in is in the
        class hierarchy. Having the explicit method keeps compatibility with
        Accelerate wrappers that may strip attributes on the unwrapped model.
        """
        return GenerationMixin.generate(self, *args, **kwargs)

    def _prepare_generation_config(self, generation_config=None, *args, **kwargs):
        """Delegate to the default GenerationMixin implementation.
        This method is looked up during GenerationMixin.generate; adding it here
        prevents attribute-missing errors when the mix-in chain is disrupted by
        wrappers such as `accelerate`.
        """
        # Accept extra positional args for compatibility with different Transformers versions
        return GenerationMixin._prepare_generation_config(self, generation_config, **kwargs)
