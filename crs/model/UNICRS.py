...
from crs.model.unicrs.model_prompt import KGPrompt
from crs.model.unicrs.model_gpt2 import PromptGPT2forCRS
from crs.model.unicrs.kg_unicrs import KGForUniCRS
from crs.model.unicrs.config import get_special_tokens_dict
...
class UNICRS():
    ...
    def __init__(self, seed, kg_dataset, debug, tokenizer_path,
        ...
        self.rec_model_path = f"crs/{rec_model}"
        self.conv_model_path = f"crs/{conv_model}"
    ...
    async def get_conv(self, conv_dict):
        ...
        # Keep special tokens (e.g., <mask>/<movie>) so downstream replacement works
        gen_str = self.tokenizer.decode(
            gen_seqs[0], skip_special_tokens=False
        )
