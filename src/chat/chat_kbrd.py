...
async def chat(user: str, data: dict, args, recommender: RECOMMENDER, metadata: dict, title_to_id: dict, id2entity: dict, save_dir: str, eval_dir: str, entity_list:list, semaphore: asyncio.Semaphore) -> None:
    async with semaphore:
        ...
        # recommender_init = "Hello"
        dialogue_history.append({
            'role': "assistant",
            'content': recommender_init
        })
        dialogue_history_kbrd["context"].append(recommender_init)
        ...
        for i in range(args.turn_num):
            ...
            dialogue_history_kbrd["context"].append(seeker_response)
            entities = get_entity(seeker_response, entity_list)
            # print(entities)
            dialogue_history_kbrd['entity'] += entities
            dialogue_history_kbrd['entity'] = list(set(dialogue_history_kbrd['entity']))
            # For Model Input
            dialogue_history.append({
                "role": "user",
            ...
            all_recommendations = await recommender.get_rec(dialogue_history_kbrd)  # Otherwise, augment dialogue history to get item list
            ...
            recommender_response = await recommender.crs_model.get_conv(dialogue_history_kbrd, candidate_list)
            
            dialogue_history_kbrd["context"].append(recommender_response)
