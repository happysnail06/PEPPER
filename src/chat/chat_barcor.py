...
async def chat(user: str, data: dict, args, recommender: RECOMMENDER, metadata: dict, title_to_id: dict, id2entity: dict, save_dir: str, eval_dir: str, semaphore: asyncio.Semaphore) -> None:
    async with semaphore:
        ...
        dialogue_history_barcor = [] # for crs
        ...
        dialogue_history_barcor.append(recommender_init)
        ...
        for i in range(args.turn_num):
            ...
            dialogue_history_barcor.append(seeker_response)
            
            ######################################  3. Update Item list ######################################
            # Retrieve Top-50 Recommendations, get_rec returns item ID
            all_recommendations = await recommender.get_rec(dialogue_history_barcor)
            ...
            recommender_response = await recommender.crs_model.get_conv(dialogue_history_barcor, candidate_list)
...
async def main():
    ...
    args = parse_args_barcor()
    # set_openai_api()
    
    # Directory for saving dialogues and recommendation list.
    save_dir = os.path.join('experiments/5_2_main/results/dialogue_{}_{}_{}'.format(args.turn_num, args.core_num, SETTING), args.crs_model, args.kg_dataset)
