...
async def chat(user: str, data: dict, args, recommender: RECOMMENDER, metadata: dict, title_to_id: dict, id2entity: dict, save_dir: str, eval_dir: str, semaphore: asyncio.Semaphore) -> None:
    async with semaphore:
        ...
        for i in range(args.turn_num):
            ...
            # ChatCRS framework: get_conv now returns (response, cost, debug_info) 
            recommender_response, cost, debug_info = await recommender.crs_model.get_conv(dialogue_history, candidate_list)
            TOTAL_COST += cost
            
            # # Print ChatCRS debug information
            # print(f"\n=== ChatCRS Debug Info - Turn {turn} ===")
            # print(f"Predicted Goal: {debug_info['dialogue_goal']}")
            # print(f"Retrieved Knowledge: {debug_info['knowledge']}")
            # print(f"Generated Response: {recommender_response}")
            # print("=" * 50)
...
async def main():
    ...
    # random_selection = list(user_data.items())[0:samples]
