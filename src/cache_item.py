...
from tenacity import _utils, Retrying, retry_if_not_exception_type, RetryCallState
...
class my_stop_after_attempt(stop_base):
    ...
    def __call__(self, retry_state: "RetryCallState") -> bool:
        if retry_state.outcome == openai.OpenAIError:
...
def annotate(item_text_list):
    ...
    for attempt in Retrying(
        reraise=True, retry=retry_if_not_exception_type((openai.OpenAIError)),
...
if __name__ == '__main__':
    ...
    save_dir = f'crs/model/chatgpt/{dataset}'
    os.makedirs(save_dir, exist_ok=True)

    with open(f'crs_data/{dataset}/id2info.json', encoding='utf-8') as f:
        id2info = json.load(f)
    ...
    if dataset == 'redial':
        ...
        attr_list = ['genre', 'star', 'director', 'plot']
    ...
    if dataset == 'opendialkg':
        ...
        for info_dict in id2info.values():
            ...
            item_texts.append(item_text)
        attr_list = ['genre', 'actor', 'director', 'writer', 'plot']
